...
int init(char configpath[], struct udevInfo* udev, struct configInfo* config, struct keyboardInfo* kbd, struct managedBuffer* device, int* epollfd, struct epoll_event* udevevent)
{
...
int main(int argc, char* argv[])
{
    ...
    if (init(arg.configpath, &udev, &config, &kbd, &device, &epollfd, &udevevent)) {
        LOG(-1, "init failed\n");
        return -1;
    }
